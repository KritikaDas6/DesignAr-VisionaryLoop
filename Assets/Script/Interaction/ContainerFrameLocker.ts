...
import {FrameInputHandler, FrameInputOptions} from "SpectaclesInteractionKit.lspkg/Components/UI/ContainerFrame/modules/FrameInputHandler"
...
import {CursorHandler} from "SpectaclesInteractionKit.lspkg/Components/UI/ContainerFrame/modules/CursorHandler"
import {HoverBehavior} from "SpectaclesInteractionKit.lspkg/Components/UI/ContainerFrame/modules/HoverBehavior"
import {LabeledPinchButton} from "SpectaclesInteractionKit.lspkg/Components/UI/ContainerFrame/modules/LabeledPinchButton"
import {SmoothFollow} from "SpectaclesInteractionKit.lspkg/Components/UI/ContainerFrame/modules/SmoothFollow"
import {SnappableBehavior} from "SpectaclesInteractionKit.lspkg/Components/UI/ContainerFrame/modules/SnappableBehavior"
...
export class ContainerFrame extends BaseScriptComponent {
  private framePrefab: ObjectPrefab = requireAsset(
    "SpectaclesInteractionKit.lspkg/Components/UI/ContainerFrame/Prefabs/FramePrefab.prefab",
  ) as ObjectPrefab
  private labeledButtonPrefab = requireAsset(
    "SpectaclesInteractionKit.lspkg/Components/UI/ContainerFrame/Prefabs/container-button.prefab",
  ) as ObjectPrefab
  private closeIcon: Texture = requireAsset(
    "SpectaclesInteractionKit.lspkg/Components/UI/ContainerFrame/Textures/close-icon-1.png",
  ) as Texture
  private followIcon: Texture = requireAsset(
    "SpectaclesInteractionKit.lspkg/Components/UI/ContainerFrame/Textures/follow-white.png",
  ) as Texture
  private unfollowIcon: Texture = requireAsset(
    "SpectaclesInteractionKit.lspkg/Components/UI/ContainerFrame/Textures/follow-black.png",
  ) as Texture
  ...
  border: number = 7
  @input
  @hint("Frame thickness/depth in Z direction (default: 0.0667)")
  thickness: number = 0.0667
  @input("vec2", "{0,0}")
  ...
  // Disabled: Follow behavior should never be used - no @input decorator to hide from inspector
  // @input
  ...
  // Keep properties public for compatibility but don't expose in inspector (no @input)
  showFollowButton: boolean = false
  private useFOVFollow: boolean = false
  isFollowing: boolean = false
  @ui.group_end
  @ui.label("")
  // Disabled: Close button should never show - no @input decorator to hide from inspector
  // @ui.group_start("Close Button")
  // @input
  // showCloseButton: boolean = false
  // @ui.group_end
  // Keep property public for compatibility but don't expose in inspector (no @input)
  showCloseButton: boolean = false
  ...
  // Disabled: Close and follow buttons should never show
  ...
  //  */
  closeButton!: LabeledPinchButton  // Keep for type safety but will be disabled
  followButton!: LabeledPinchButton  // Keep for type safety but will be disabled
  ...
  onAwake() {
    ...
    // })
    // if (this.closeButton && this.closeButton.object) {
    ...
    // })
    // if (this.followButton && this.followButton.object) {
    //   this.followButton.object.enabled = false
    // }
    
    // Create dummy buttons to prevent errors but keep them completely hidden
    this.closeButton = new LabeledPinchButton({
      prefab: this.labeledButtonPrefab,
      parent: this.frame,
      labels: [this.closeIcon],
    ...
    if (this.closeButton && this.closeButton.object) {
      this.closeButton.object.enabled = false
      // Hide by scaling to zero and moving far away
      this.closeButton.transform.setLocalScale(new vec3(0, 0, 0))
      this.closeButton.transform.setLocalPosition(new vec3(0, 0, -1000))
    }
    this.followButton = new LabeledPinchButton({
      prefab: this.labeledButtonPrefab,
      parent: this.frame,
    ...
    if (this.followButton && this.followButton.object) {
      this.followButton.object.enabled = false
      // Hide by scaling to zero and moving far away
      this.followButton.transform.setLocalScale(new vec3(0, 0, 0))
      this.followButton.transform.setLocalPosition(new vec3(0, 0, -1000))
    }
    ...
    // Disabled: Follow behavior should never be used
    ...
    // Disabled: Close and follow buttons should never be interactable
  ...
  /**
   * Set up the lock interactable to trigger lock/unlock functionality
   */
  private setupLockInteractable() {
    if (this.lockInteractable) {
      // Use onTriggerEnd for button press actions
      if (this.lockInteractable.onTriggerEnd && this.lockInteractable.onTriggerEnd.add) {
        this.lockInteractable.onTriggerEnd.add(() => {
          this.toggleLock()
        })
        log.d("Successfully set up lock interactable")
      } else {
        log.w("Lock interactable does not have onTriggerEnd.add method")
      }
    } else {
  ...
  lock() {
    this.isLocked = true
    log.d("ContainerFrameLocker locked")
  ...
  unlock() {
    this.isLocked = false
    log.d("ContainerFrameLocker unlocked")
  ...
  private scaleFrame = () => {
    ...
    this.material.mainPass.scaleZ = this.thickness
  ...
  private scaleAndPositionButtons = () => {
    // Disabled: Button scaling and positioning - buttons should never show
    ...
    // Disabled: CORNER POSITIONING for buttons
  ...
  /**
   * Reset inner size and content scale to default values
   * This resets both the frame size and the content scale inside the frame
   * @param newSize The new inner size (default: 14, 14)
   */
  resetInnerSizeAndContent = (newSize: vec2 = new vec2(14, 14)) => {
    // Reset inner size
    this.innerSize = newSize
    
    // Reset original scale to match new size so content scale factor is 1.0
    this.originalScale = new vec2(newSize.x, newSize.y)
    this.targetScaleCache = new vec2(newSize.x, newSize.y)
    
    // Update interaction plane
    this.interactionPlane.planeSize = this.totalInnerSize.add(
      vec2.one().uniformScale(this.border * 2),
    )
    
    // Force update the frame and content scale
    this.forcePreserveScale = false
    this.scaleFrame()
  }
  ...
  set renderOrder(renderOrder: number) {
    ...
    // Disabled: Buttons should never show, so no need to set render order
  ...
  set opacity(opacity: number) {
    ...
    if (!this.destroyed) {
      ...
      // Disabled: Buttons should never show, so no need to set alpha
  ...
  enableCloseButton = (enabled: boolean) => {
    ...
    // Disabled: Close button should never show
    // this.closeButton.object.enabled = enabled
    // Always keep close button disabled
    if (this.closeButton) this.closeButton.object.enabled = false
  ...
  enableFollowButton = (enabled: boolean) => {
    ...
    // Disabled: Follow button should never show
    // this.followButton.object.enabled = enabled
    // Always keep follow button disabled
    if (this.followButton) this.followButton.object.enabled = false
    const scaleHandles = this.material.mainPass.scaleHandles
