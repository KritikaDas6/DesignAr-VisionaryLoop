...
activated."
  ...
  /**
   * Material to swap with meshIdleMaterial on pinch. When enabled, each pinch will toggle between the original 
   * idle material and this swap material.
   */
  @ui.separator
  @input("Asset.Material")
  @hint("Material to swap with meshIdleMaterial on pinch. Each pinch toggles between original and swap materials.")
  @allowUndefined
  swapMaterial: Material | undefined

  /**
   * When enabled, the material swapping feature will be active. Each pinch will toggle between the original 
   * idle material and the swap material.
   */
  @input
  @hint("When enabled, each pinch will toggle between the original idle material and swap material.")
  enableMaterialSwap: boolean = false
  ...
  private originalIdleMaterial: Material | undefined
  ...
  private init = (): void => {
    ...
    this.createEvent("OnStartEvent").bind(() => {
      ...
      // Initialize material swapping if enabled
      if (this.enableMaterialSwap && this.swapMaterial) {
        this.originalIdleMaterial = this.meshIdleMaterial.clone()
        print("ButtonFeedbackSwap: Material swapping enabled with swap material: " + this.swapMaterial.name)
      } else if (this.enableMaterialSwap && !this.swapMaterial) {
        print("ButtonFeedbackSwap: Material swapping enabled but no swap material assigned!")
        this.enableMaterialSwap = false
      }
  ...
  private initializeTriggeredState_PinchButton = (): void => {
    // Handle material swapping if enabled
    if (this.enableMaterialSwap && this.swapMaterial && this.originalIdleMaterial) {
      this.isSwapped = !this.isSwapped;
      if (this.isSwapped) {
        this.meshIdleMaterial = this.swapMaterial;
        print("ButtonFeedbackSwap: Swapped to swap material");
      } else {
        this.meshIdleMaterial = this.originalIdleMaterial;
        print("ButtonFeedbackSwap: Swapped back to original material");
      }
    }
  ...
  /**
   * Public method to reset button to idle state
   * Resets the material to meshIdleMaterial and resets toggle state if applicable
   * IMPORTANT: If material swapping is enabled, restores the original idle material
   */
  public resetToIdle = (): void => {
    try {
      // Reset toggle state if it's a toggle button
      if (this.toggleButton && (this.toggleButton as any).isToggledOn) {
        (this.toggleButton as any).isToggledOn = false
      }
      
      // If material swapping is enabled, restore original idle material first
      if (this.enableMaterialSwap && this.originalIdleMaterial) {
        this.meshIdleMaterial = this.originalIdleMaterial
        this.isSwapped = false
        print("ButtonFeedback_Modified: resetToIdle() - Restored original idle material (material swap reset)")
      }
      
      // Reset to idle material
      if (this.meshIdleMaterial) {
        this.changeButtonState(this.meshIdleMaterial)
      }
      
      // Reset glow if applicable
      if (this.useGlowMesh && this.glowIdleMaterial) {
        this.changeGlowState(this.glowIdleMaterial)
      }
    } catch (e) {
      print("ButtonFeedback_Modified: Error resetting to idle - " + e)
    }
  }
}
