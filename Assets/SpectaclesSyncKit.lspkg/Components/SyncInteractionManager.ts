...
import {InteractableHitInfo} from "../../SpectaclesInteractionKit/Providers/TargetProvider/TargetProvider"
...
export class SyncInteractionManager extends BaseScriptComponent {
  ...
  // Simplified update method that doesn't try to intercept InteractionManager events
  private update() {
    // For now, we'll use a simpler approach that doesn't require intercepting InteractionManager events
    // This can be enhanced later if needed
  }

  // Simplified store update processing
  ...
  private parseEventDispatch(eventStringArray: string[]): SyncInteractorEvent[] {
    ...
    for (const eventString of eventStringArray) {
      ...
      const event: SyncInteractorEvent = {
        connectionId: "", // This would need to be passed separately or stored differently
        eventName: eventName,
        targetId: targetId,
  ...
  // Simplified method to find interactables - this would need to be implemented based on your specific needs
  private findInteractableById(id: string): Interactable | null {
    // This is a simplified implementation. You might need to implement a more sophisticated
    // way to find interactables by ID depending on your specific requirements.
    return null
  }
}
...
function serializeInteractable(interactable: Interactable | null): InteractableSerialized {
  ...
  return {
    id: interactable.sceneObject.name // Using sceneObject name as ID for now
  }
}
...
function parseInteractable(object: InteractableSerialized): Interactable | null {
  ...
  // This would need to be implemented based on how you want to find interactables
  return null
}
...
function parseRaycastHit(object: RayCastHitSerialized): RayCastHit | null {
  ...
  // This would need to be implemented based on how you want to reconstruct RayCastHit
  return null
}
...
function parseInteractableHitInfo(object: InteractableHitInfoSerialized): InteractableHitInfo | null {
  ...
  // This would need to be implemented based on how you want to reconstruct InteractableHitInfo
  return null
}
...
function serializeInteractor(interactor: Interactor): InteractorSerialized {
  return {
    ...
    distanceToTarget: interactor.distanceToTarget || 0,
    ...
    interactionStrength: interactor.interactionStrength || 0,
    ...
    dragType: interactor.dragType || 0,
