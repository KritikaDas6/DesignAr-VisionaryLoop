...
export class JoiningController {
  ...
  startEventsMonitoring(): void {
    SessionController.getInstance().onUserJoinedSession.add((session, userInfo) => {
      if (
        userInfo.displayName === "STUDIO_CONNECTED_LENS_MONITOR" ||
        SessionController.getInstance().getCustomLandmark() !== null
      ) {
        return
      }
      ...
      if (this.projectContainer.isMappedObservable.value) {
        if (this.stateMachine.currentState.name !== TeachingJoinedUserState.name) {
    ...
    SessionController.getInstance().onMessageReceived.add((session, userId, message, senderInfo) => {
  ...
  getUserInfoIndex(userInfo: ConnectedLensModule.UserInfo): number {
