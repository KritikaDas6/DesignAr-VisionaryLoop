...
import animate, {CancelFunction, easingFunctions} from "../../../../../SpectaclesInteractionKit/Utils/animate"
...
export default class HeadLockBehavior {
  ...
  private readonly cameraTransform: Transform = this.worldCameraObject.getTransform()
  ...
  private currentCounterToSkipPositionUpdate = 0

  private objectIsMoving = false
  ...
  start(needToBeAnimatedToStartPos = false): void {
    ...
    if (needToBeAnimatedToStartPos) {
      ...
      this.cancelFunction = this.animateToPosition(
        ...
        newObjectPos,
        lerp(0, this.objectMovingDurationOnStart, clamp(distance, 0, this.maxDistance) / this.maxDistance),
        () => {
  ...
  private moveObjectToFOV(onComplete: () => void, currentScale?: vec3) {
    ...
    this.cancelFunction = this.animateObjectMovingToFOV(
      this.objectTransform,
      lerp(0.5, this.objectMovingDurationReturnBack, clamp(distance, 0, this.maxDistance) / this.maxDistance),
      onComplete,
      "ease-out-back-cubic"
    )
  }
  ...
  private needToUpdateObjectPosition(offsetAngle: number, offsetDistance: number): boolean {
    const needToUpdate =
      offsetAngle > this.maxOffsetFromCenter || offsetDistance > this.config.bufferTranslationCentimeters
    if (needToUpdate) {
  ...
  private updateObjectRotation(immediatelyUpdate = false) {
    ...
    if (immediatelyUpdate) {
      ...
      this.objectTransform.setWorldRotation(
        quat.slerp(this.objectTransform.getWorldRotation(), newRot, 6 * getDeltaTime())
      )
    }
  }
  ...
  private animateObjectMovingToFOV(
    ...
    duration: number,
    onComplete: () => void = () => {},
    easing: keyof typeof easingFunctions | null = null
  ): CancelFunction {
    const startPosition = transform.getWorldPosition()
    return animate({
      update: (t: number) => {
