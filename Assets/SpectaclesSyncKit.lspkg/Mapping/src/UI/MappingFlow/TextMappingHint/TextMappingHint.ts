...
export class TextMappingHint {
  ...
  start(hintSet: ITitledText[], delayBeforeStart: number): void {
  ...
  private showText(id: number) {
    ...
    this.delayEvent.bind(() => {
      ...
      this.textAnimation = this.animateToAlpha(this.input.root, 0, 1, this.animationParameters.fadeInTime, () =>
        this.hideText(id)
      )
    })
  ...
  private hideText(id: number) {
    this.delayEvent = this.input.script.createEvent("DelayedCallbackEvent")
    this.delayEvent.bind(() => {
      this.textAnimation = this.animateToAlpha(this.input.root, 1, 0, this.animationParameters.fadeOutTime, () =>
